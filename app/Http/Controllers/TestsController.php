...
class TestsController extends Controller
{
    public function getTestQuestions(Request $request, $subject_id)
    {
        ...
        $student_already_registered = Student::where('user_id', Auth::user()->id)
            ->where('subject_id', $subject_id)
            ->exists();
        $has_taken_exam = Result::where('user_id', Auth::user()->id)
            ->where('subject_id', $subject_id)
            ->exists();
        ...
        if ($has_taken_exam) {
            ...
            if ($has_taken_exam) {
                ...
                $questions = Test::where('subject_id', $subject_id)
                    ->get();
                $exam_deadline = Subject::where('id', $subject_id)
                    ->value('exam_deadline');
                //if students starts the exam for the first time then return this
                $duration = Subject::where('id', $subject_id)
                    ->value('duration');
                return view('test', [
                    'questions' => $questions,
    ...
    public function allResults()
    {
        ...
        $allResults = Result::where('user_id', Auth::user()->id)
            ->get();
    ...
    public function registerExam(Request $request, $subject_id)
    {
        ...
        $student_already_register = Student::where('user_id', Auth::user()->id)
            ->where('subject_id', $subject_id)
            ->exists();
        $has_taken_exam = Result::where('user_id', Auth::user()->id)
            ->where('subject_id', $subject_id)
    ...
    public function submitExam(Request $request)
    {
        ...
        foreach ($answers as $questionId => $userAnswer) { //loop array
            ...
            if (is_numeric($questionId)) {
                $questionInfo = Test::where('id', $questionId)->get();
